...
from app.common.local_search import (
    is_local_backend,
    local_calendar_event_tokens,
    local_calendar_terms,
    local_tuition_filter_values,
    tuition_documents,
)
...
def get_calendar_terms() -> List[str]:
    ...
    if is_local_backend():
        return _post_process_calendar_terms(local_calendar_terms())
...
def get_calendar_event_tokens() -> List[str]:
    ...
    if is_local_backend():
        return local_calendar_event_tokens()
...
_TUITION_INDEX = "tuition_fees"


def get_tuition_schools() -> List[str]:
    if is_local_backend():
        return local_tuition_filter_values()["schools"]
    return _agg_terms(_TUITION_INDEX, "school")


def get_tuition_levels() -> List[str]:
    if is_local_backend():
        levels = {doc.get("level") for doc in tuition_documents() if doc.get("level")}
        return sorted(level for level in levels if level != "all")
    return [l for l in _agg_terms(_TUITION_INDEX, "level") if l != "all"]


def get_tuition_years() -> List[str]:
    if is_local_backend():
        return sorted(local_tuition_filter_values()["years"], reverse=True)
    return sorted(_agg_terms(_TUITION_INDEX, "academic_year"), reverse=True)


def get_tuition_fee_names() -> List[str]:
    if is_local_backend():
        return local_tuition_filter_values()["fee_names"]
